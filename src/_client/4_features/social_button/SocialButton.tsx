...
export function SocialButton({ loginType, disabled }: SocialButtonProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "bg-secondary border-border focus:border-primary focus:outline-primary/25 text-secondary-foreground flex h-[48px] w-full items-center justify-center rounded-md border-[1px] py-3 outline-transparent transition-all duration-200 ease-out focus:outline-[3px] active:scale-98",
        {
          "bg-muted text-muted-foreground": disabled,
          "hover:bg-secondary/80 hover:cursor-pointer": !disabled,
        },
      )}
    >
